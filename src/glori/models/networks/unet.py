...
from glori.models.networks.modules import configModuleBase
...
class UnetTimeEmb(configModuleBase):
    ...
    def forward(
        ...
        if (
            ...
            ctxt_dropout_mask = torch.rand(x.shape[0], device=x.device)
            ctxt_dropout_mask = ctxt_dropout_mask >= self.context_dropout
        ...
        if self.label_emb is not None and class_labels is not None:
            ...
            if self.training and self.label_dropout:
                labels_enc = labels_enc * ctxt_dropout_mask.view(-1, 1).to(
                    labels_enc.dtype
                )
        ...
        if self.context_emb is not None and context is not None:
            ...
            if self.training and self.context_dropout:
                context_emb = context_emb * ctxt_dropout_mask.view(-1, 1).to(
                    context_emb.dtype
                )
        ...
        if img_context is not None:
            if self.training and self.context_dropout:
                img_context = img_context * ctxt_dropout_mask.view(-1, 1, 1, 1).to(
                    x.dtype
                )
        ...
        if self.use_catalog_ctxt:
            if self.training and self.context_dropout:
                catalog_context = catalog_context * ctxt_dropout_mask.view(
                    -1, 1, 1, 1
                ).to(x.dtype)
