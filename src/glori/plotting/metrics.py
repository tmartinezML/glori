...
from glori.plotting.utils import (
    add_distribution_delta_plot,
...
from glori.analysis.stats_utils import norm
...
def pixel_metrics_plot(
    distributions_dict, pixel_dist=None, fig_ax=None, color=None, label=None
):
    ...
    assert (
        pixel_dist is not None or "Pixel_Intensity" in distributions_dict
    ...
    # Keys to plot
    keys = ["Bin_Pixels", "Image_Mean", "Bin_Mean", "Image_Sigma", "Bin_Sigma"]
    color = color if color is not None else "black"

    # Assert all keys are present in distributions_dict
    assert all(mask := [k in distributions_dict for k in keys]), (
        f"Not all keys are present in distributions_dict:" f" {compress(keys, ~mask)}"
    )

    # Plot pixel distribution first
    ax = axs[0]
    add_distribution_plot(
        pixel_dist, np.linspace(0, 1, len(pixel_dist) + 1), ax, label=label, color=color
    )
    ...
    for key in keys:
        if key.startswith("Bin_"):
            ...
            for sub_key in sub_dict.keys():
                ...
                add_distribution_plot(
                    counts, edges, axs[i], label=str(sub_key), color=color
                )
                if "Mean" in key:
            ...
            add_distribution_plot(counts, edges, axs[i], label=label, color=color)
...
def shape_metrics_plot(
    distributions_dict, COM=None, fig_ax=None, color=None, label=None
):
    if fig_ax is None:
        fig, axs = plt.subplots(3, 2, figsize=(16, 16), dpi=150, tight_layout=True)
    else:
        fig, axs = fig_ax

    # Assert COM is either in dict or passed
    assert (
        COM is not None or "COM" in distributions_dict
    ), "COM should be passed or be present in distributions_dict."
    COM = COM or distributions_dict["COM"][0]

    # Keys to plot
    keys = ["WPCA_Angle", "WPCA_Elongation", "COM_Radius", "COM_Angle", "Scatter"]
    color = color if color is not None else "black"

    for ax, key in zip(axs.flatten()[:-1], keys):
        counts, edges = distributions_dict[key]
        add_distribution_plot(counts, edges, ax, label=label, color=color)
...
def metrics_plots(distr, labels=None, colors=None, cmap="cividis", **plot_kwargs):
    ...
    for plot_fnc in [pixel_metrics_plot, shape_metrics_plot]:
        match distr:
            case list():
                assert (
                    labels is None or type(labels) == list
                ), "If distr is a list, labels should be a list as well."
                assert (
                    colors is None or type(colors) == list
            ...
            case dict():
                assert (
                    labels is None or type(labels) == str
                ), "If distr is a dictionary, labels should be a string."
                assert (
                    colors is None or type(colors) == str
                ), "If distr is a dictionary, colors should be a string."
                out.append(
                    plot_fnc(
                        distr, fig_ax=None, color=colors, label=labels, **plot_kwargs
                    )
                )
    return out
...
def plot_distributions(
    ...
    for ax, ax2, key in zip(
        ...
        if isinstance(distr_gen, dict):  # Single dictionary
            ...
            assert all(
                isinstance(d, dict) for d in distr_gen
        ...
        add_distribution_plot(
            c_lofar, e_lofar, ax, color="tab:orange", alpha=0.7, label=f"Real"
        )

        # Set properties for distributions plot
        # (x-axis limits, labels, log-scale, grid, legend)
        xmax = np.max(
            np.concatenate([e_gen[1:][c_gen > 0], e_lofar[1:][c_lofar > 0]])
        ).item()
        ...
        if isinstance(error, dict):  # Single dictionary
            ...
            assert all(
                isinstance(d, dict) for d in error
...
def plot_W1_distances(W1_dict_list, x_values=None, x_label=""):
    ...
    # Create figure and axes
    fig, ax = plt.subplots(1, 1, figsize=(10, 6), dpi=150, tight_layout=True)
...
def bdsf_metrics_plot(distributions_dict, fig_ax=None, color=None, label=None):
    ...
    # Keys to plot
    keys = ["total_flux_gaus", "ngaus", "nsrc", "q0.9_area", "q0.5_area", "q0.1_area"]
    color = color if color is not None else "black"
